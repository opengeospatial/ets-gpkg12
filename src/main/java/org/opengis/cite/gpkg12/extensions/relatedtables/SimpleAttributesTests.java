 ...
 * </p>
 * <ul>
 * <li><a href="http://docs.opengeospatial.org/is/18-000/18-000.html#sa_rc" target=
 * "_blank"> GeoPackage Related Tables Extension, Simple Attributes Extension</a> (OGC
 * 18-000)</li>
 * </ul>
 *
 * @author Jeff Yutzler
...
public class SimpleAttributesTests extends RTEBase {
	...
	/**
	 * A user-defined related data table or view SHALL be a user-defined simple attribute
	 * table or view if the row in gpkgext_relations with a corresponding
	 * related_table_name has a relation_name of "simple_attributes".
	...
	/**
	 * A user-defined simple attribute table or view SHALL contain the primary key column
	 * and at least one other column as defined in User-Defined Simple Attributes Table
	 * Definition. A user-defined simple attribute table SHALL only contain data belonging
	 * to the TEXT, INTEGER, or REAL storage classes and SHALL NOT contain NULL or BLOB
			...
			while (resultSet.next()) {
				...
				try (final Statement statement2 = this.databaseConnection.createStatement();
						...
						final ResultSet resultSet2 = statement2
							.executeQuery(String.format("PRAGMA table_info(%s)", simpleAttributesTableName));) {
					while (resultSet2.next()) {
						final String type = resultSet2.getString("type");
						final String name = resultSet2.getString("name");
						switch (type) {
							case "BLOB":
								Assert.fail(ErrorMessage.format(ErrorMessageKeys.INVALID_DATA_TYPE, type, name,
