 ...
 * Encoding Standard - Contents</a> (OGC 12-128r14)</li>
 * </ul>
 *
 * @author Luke Lambert
 */
public class DataContentsTests extends CommonFixture {
	 ...
	 * Types</a>.
	 *
	 * @see <a href="http://www.geopackage.org/spec/#r5" target= "_blank">File Contents -
	 * Requirement 5</a>
					...
					try (final Statement preparedStatement = this.databaseConnection.createStatement();
						...
						while (pragmaTableInfo.next()) {
							...
							final boolean correctDataType = isExtendedType(tableName, columnName)
									|| getAllowedSqlTypes().contains(dataType) || TEXT_TYPE.matcher(dataType).matches()
									|| BLOB_TYPE.matcher(dataType).matches();

							// 3bi
							assertTrue(correctDataType, ErrorMessage.format(ErrorMessageKeys.INVALID_DATA_TYPE,
	 ...
	 * Definition SQL</a>.
	 *
	 * @see <a href="http://www.geopackage.org/spec/#_r13" target= "_blank">Table
	 * Definition - Requirement 13</a>
	 * @throws SQLException If an SQL query causes an error
	 */
	 ...
	 *
	 * @see <a href="http://www.geopackage.org/spec/#r14" target= "_blank">Table Data
	 * Values - Requirement 14</a>
	 * @throws SQLException If an SQL query causes an error
	 */
	 ...
	 *
	 * @see <a href="http://www.geopackage.org/spec/#r15" target= "_blank">Table Data
	 * Values - Requirement 15</a>
	 * @throws SQLException If an SQL query causes an error
	 */
	...
	public void timestampFormat() throws SQLException {
		...
		try (final Statement statement = this.databaseConnection.createStatement();
			...
			// 4
			assertTrue(invalidDateTableNames.isEmpty(), ErrorMessage.format(
					ErrorMessageKeys.BAD_CONTENTS_ENTRY_LAST_CHANGE_FORMAT, String.join(", ", invalidDateTableNames)));
		}
	}
	 ...
	 * srs_id} column.
	 *
	 * @see <a href="http://www.geopackage.org/spec/#r16" target= "_blank">Table Data
	 * Values - Requirement 16</a>
	 * @throws SQLException If an SQL query causes an error
	 */
	...
	/**
	 * Verify that a GeoPackage contains a features or tiles table and gpkg_contents table
	 * row describing it.
	 *
	 * @see <a href="http://www.geopackage.org/spec/#_r17" target= "_blank">Options -
	 * Requirement 17</a>
	 * @throws SQLException If an SQL query causes an error
	 */
	...
	public void optValidGeoPackage() throws SQLException {
		...
		// try(final Statement statement = this.databaseConnection.createStatement();
		// final ResultSet resultSet = statement.executeQuery("SELECT COUNT(*) FROM
		// gpkg_contents WHERE data_type IN ('tiles', 'features')"))
		// {
		// resultSet.next();
	...
	private static final Collection<String> ALLOWED_SQL_TYPES = Arrays.asList("BOOLEAN", "TINYINT", "SMALLINT",
			...
			"LINESTRING", "POLYGON", "MULTIPOINT", "MULTILINESTRING", "MULTIPOLYGON", "GEOMETRYCOLLECTION", "INTEGER");
